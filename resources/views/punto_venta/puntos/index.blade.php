...
.pdv-tile__meta { display: flex; flex-wrap: wrap; gap: .4rem; margin-bottom: .85rem; }
.pdv-status-pill {
...
.pdv-status-pill--activo { background: linear-gradient(135deg, #059669, #10b981); color: #fff; box-shadow: 0 2px 8px rgba(5,150,105,.25); }
.pdv-status-pill--inactivo { background: #e2e8f0; color: #64748b; }
.pdv-status-pill--stock { background: linear-gradient(135deg, #0284c7, #0ea5e9); color: #fff; box-shadow: 0 2px 8px rgba(2,132,199,.2); }
.pdv-status-pill--sin-stock { background: #f8fafc; color: #94a3b8; border: 1px dashed #cbd5e1; }
.pdv-tile__addr {
            ...
            @forelse($puntos as $punto)
                <article class="pdv-tile {{ ! $punto->activo ? 'pdv-tile--inactivo' : '' }}">
                    ...
                    <div class="pdv-tile__meta">
                        <span class="pdv-status-pill {{ $punto->activo ? 'pdv-status-pill--activo' : 'pdv-status-pill--inactivo' }}">
                            <i class="fas {{ $punto->activo ? 'fa-check-circle' : 'fa-pause-circle' }}"></i>
                            {{ $punto->activo ? 'Operativo' : 'Fuera de servicio' }}
                        </span>
                        <span class="pdv-status-pill {{ $punto->almacenid ? 'pdv-status-pill--stock' : 'pdv-status-pill--sin-stock' }}">
                            <i class="fas {{ $punto->almacenid ? 'fa-boxes' : 'fa-box-open' }}"></i>
                            {{ $punto->almacenid ? 'Inventario activo' : 'Sin inventario' }}
                        </span>
                    </div>
