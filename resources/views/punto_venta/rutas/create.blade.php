...
<div class="card card-outline card-success">
    <div class="card-header">
        <h3 class="card-title font-weight-bold mb-0"><i class="fas fa-route mr-2"></i>Armar ruta mayorista → minoristas</h3>
    </div>
    <div class="card-body">
        @if(session('error'))
        ...
        <form method="POST" action="{{ route('punto-venta.rutas.store') }}" id="formRutaDistribucion">
            ...
            <div class="alert alert-info py-2 small">
                ...
                Elija el <strong>almacén mayorista</strong> donde cargará (solo aparecen centros con pedidos listos),
                marque los pedidos de minoristas a entregar y asigne un <strong>transportista de flota mayorista</strong>.
                ...
                <div class="form-group col-lg-4 mb-lg-0">
                    <div class="ruta-flota-panel ruta-flota-panel--origen">
                        <label class="small font-weight-bold mb-2 d-block">
                            ...
                            Almacén mayorista (carga) <span class="text-danger">*</span>
                        </label>
                        <div class="input-group mb-2">
                            <input type="text" id="txtAlmacenOrigen" class="form-control bg-white" readonly
                                   placeholder="Buscar almacén mayorista…" value="{{ $oldAlmacenLabel }}" required>
                            <div class="input-group-append">
                                <button type="button" class="btn btn-warning" id="btnBuscarAlmacenOrigen" title="Buscar mayorista">
                                    <i class="fas fa-search"></i>
                                </button>
                            </div>
                        </div>
                        <input type="hidden" name="almacen_mayorista_origenid" id="almacen_origen" value="{{ old('almacen_mayorista_origenid') }}" required>
                        <p class="small text-muted mb-0">Solo se listan centros mayoristas con pedidos de minoristas pendientes de reparto.</p>
                    </div>
                </div>
                <div class="form-group col-lg-4 mb-lg-0">
                    <div class="ruta-flota-panel ruta-flota-panel--chofer">
                        ...
                        <div class="input-group">
                            <input type="text" id="txtTransportistaPlanta" class="form-control bg-white" readonly placeholder="Buscar transportista mayorista…" required>
                            <div class="input-group-append">
            ...
            <div class="row mb-3">
                <div class="form-group col-lg-4 mb-lg-0">
                    ...
                    <p class="small text-muted mb-0 mt-1">Un solo monto por la ruta completa mayorista → puntos de venta.</p>
                </div>
            </div>

            <h5 class="font-weight-bold mb-2"><i class="fas fa-store text-danger mr-1"></i>Pedidos a incluir</h5>
            <p class="small text-muted">Al elegir el centro mayorista solo verá los pedidos que salen de ese almacén. Marque el orden de visita (de arriba hacia abajo).</p>
                ...
                <table class="table table-sm ruta-pedidos-table" id="tablaPedidosRuta">
                    ...
                    <tbody id="pedidos-sortable">
                        ...
                        <tr data-pedido-id="{{ $pedido->pedidodistribucionid }}"
                            ...
                            data-almacen-origen-id="{{ $pedido->almacen_mayorista_origenid }}"
                            ...
                            <td>{{ $pedido->almacenMayoristaOrigen?->nombre ?? '—' }}</td>
                            <td>{{ $pdv?->nombre ?? '—' }}</td>
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-between mb-2 gap-2">
                ...
                <div class="ruta-mapa-leyenda d-none" id="leyendaMapaDist">
                    <span><i class="dot-planta"></i>Mayorista (clic para elegir origen)</span>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function pintarUbicacionesDisponibles() {
        ...
        almacenes.forEach(function (item) {
            ...
            marker._distItem = { tipo: 'mayorista', id: item.id };
            bounds.push([lat, lng]);
        });
    ...
    async function redibujarMapa() {
        ...
        if (puntos.length === 0) {
            resumenEl.textContent = state.ubicacionesVisibles
                ? 'Haga clic en un centro mayorista (naranja) o PDV (azul) para armar la ruta.'
                : 'Use «Ver en mapa» para ver mayoristas y PDV con pedidos, o seleccione arriba.';
    ...
    if (window.CatalogoSelector) {
        ...
        CatalogoSelector.register('ruta_dist_almacen', {
            ...
            title: 'Almacén mayorista (origen)',
            searchPlaceholder: 'Nombre o ubicación…',
            searchLabel: 'Buscar centro con pedidos',
    ...
    document.getElementById('formRutaDistribucion').addEventListener('submit', function (e) {
        if (!inputAlmacen.value) {
            e.preventDefault();
            aviso('Seleccione el almacén mayorista donde se cargará la mercadería.', 'Falta origen', 'warning');
            return;
        }
        if (!inputTransportista.value) {
            e.preventDefault();
            aviso('Asigne un transportista de flota mayorista.', 'Falta chofer', 'warning');
            return;
        }
    ...
    document.getElementById('mapaDistResumen').textContent =
        'Use «Ver en mapa» para ver centros mayoristas y PDV con pedidos disponibles.';
});
</script>
@endpush
@endif
