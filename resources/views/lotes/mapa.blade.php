...
<div class="mapa-kpi-panel">
    <div class="mapa-kpi-grid">
        <div class="mapa-kpi-card mapa-kpi-card--total">
            ...
            <div class="mapa-kpi-card__icon" aria-hidden="true"><i class="fas fa-map"></i></div>
        </div>
        <div class="mapa-kpi-card mapa-kpi-card--produccion">
            ...
            <div class="mapa-kpi-card__icon" aria-hidden="true"><i class="fas fa-leaf"></i></div>
        </div>
        <div class="mapa-kpi-card mapa-kpi-card--cosecha">
            ...
            <div class="mapa-kpi-card__icon" aria-hidden="true"><i class="fas fa-warehouse"></i></div>
        </div>
        <div class="mapa-kpi-card mapa-kpi-card--hectareas">
            ...
            <div class="mapa-kpi-card__icon" aria-hidden="true"><i class="fas fa-ruler-combined"></i></div>
        </div>
    </div>
</div>
                        ...
                        <div class="filter-group">
                            ...
                            <select class="form-control form-control-sm" id="filtroEstado">
                                ...
                                @foreach($estados as $e)
                                    @php $slugEstado = \App\Support\EstadoLoteCatalogo::slugFromNombre($e->nombre); @endphp
                                    <option value="{{ $e->estadolotetipoid }}">
                                        {{ $slugEstado ? \App\Support\EstadoLoteCatalogo::label($slugEstado) : ucfirst($e->nombre) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
...
var leyendaEstadosMapa = @json($leyendaMapa);
...
function claseEstadoBadge(estado) {
    ...
    if (norm.indexOf('finaliz') >= 0) return 'status-descanso';
    if (norm.indexOf('listo') >= 0 || norm.indexOf('certif') >= 0) return 'status-preparacion';
    if (norm.indexOf('crecim') >= 0 || norm.indexOf('producc') >= 0) return 'status-produccion';
    if (norm.indexOf('sembr') >= 0) return 'status-sembrado';
    if (norm.indexOf('planif') >= 0 || norm.indexOf('prepar') >= 0 || norm.indexOf('dispon') >= 0) return 'status-disponible';
...
function dibujarTarjetasResumenPdf(pdf, lotes, y) {
    ...
    var tarjetas = [
        ...
        { label: 'En crecimiento', valor: String(resumen.enProduccion), color: [22, 163, 74] },
...
function dibujarLeyendaEstadosPdf(pdf, y) {
    var margen = 12;
    var items = leyendaEstadosMapa.map(function (item) {
