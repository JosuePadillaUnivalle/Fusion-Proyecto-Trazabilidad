...
(function () {
    ...
    let detalleConfirmadoTipoId = '';
    let modalDetalleAbierto = false;
    const actDetError = document.getElementById('actDetError');
    ...
    function slugDesdeTipoSelect() {
        if (!selTipo || !selTipo.value) return '';
        const opt = selTipo.options[selTipo.selectedIndex];
        return slugDesdeTipoNombre(opt ? opt.textContent.trim() : '');
    }

    function tipoRequiereDetalle(slug) {
        return slug === 'riego'
            || slug === 'fertilizantes'
            || slug === 'pesticidas'
            || slug === 'material_siembra';
    }

    function detalleEstaCompleto(slug) {
        if (slug === 'riego') {
            return estado.modo === 'riego' && estado.riego && estado.riego.key;
        }
        if (slug === 'fertilizantes' || slug === 'pesticidas' || slug === 'material_siembra') {
            return estado.modo === 'insumos' && Array.isArray(estado.insumos) && estado.insumos.length > 0;
        }
        return true;
    }
    ...
    function mostrarErrorDetalle(mensaje) {
        ...
        actDetError.textContent = mensaje;
        actDetError.classList.remove('d-none');
    }

    function limpiarErrorDetalle() {
        actDetError?.classList.add('d-none');
    }

    function limpiarDetalle() {
        estado = { modo: null, insumos: [], riego: null };
        detalleConfirmadoTipoId = '';
        persistir();
    }

    function revertirTipoSinDetalle() {
        if (!selTipo || !modalDetalleAbierto) return;
        modalDetalleAbierto = false;
        const slug = slugDesdeTipoSelect();
        if (!tipoRequiereDetalle(slug) || detalleEstaCompleto(slug)) {
            return;
        }
        selTipo.value = '';
        limpiarDetalle();
        mostrarErrorDetalle('Debe confirmar el detalle (riego o insumos) antes de continuar.');
    }
    ...
    function renderCalcInsumo(container, sug, productoNombre) {
        ...
        const titulo = productoNombre
            ? 'Cantidad sugerida para todo el lote — ' + productoNombre
            : 'Cantidad sugerida para todo el lote';
        container.innerHTML =
            '<div class="small font-weight-bold text-success mb-2"><i class="fas fa-calculator mr-1"></i>' + titulo + '</div>' +
            ...
            '<div class="act-det-calc__total">' +
                '<div class="small text-muted mb-1">Total sugerido para este lote</div>' +
                '<div class="act-det-calc__total-num">' + total + ' ' + unidad + '</div>' +
                '<div class="act-det-calc__total-hint">(' + ha + ' ha × ' + porHa + ' ' + unidad + '/ha)</div>' +
    ...
    function renderCalcSiembra(container, sug) {
        renderCalcInsumo(container, sug, null);
        if (sug && sug.tiene_dosis) sugerenciaActual = sug;
    }
    ...
    function pintarInsumosModal(items, meta) {
        ...
        if (tipoSlugActual === 'material_siembra') {
        ...
        items.forEach(function (item) {
            ...
            card.addEventListener('click', function () {
                ...
                if (maxInsumos === 1) {
                    ...
                    renderCalcInsumo(calc, item.sugerencia, item.nombre);
                    sugerenciaActual = item.sugerencia || null;
                    ...
                    if (ids.length === 1) {
                        renderCalcInsumo(calc, ids[0].sugerencia, ids[0].nombre);
                    } else if (calc) {
                        calc.classList.add('d-none');
                    }
    ...
    function pintarCantidades(items) {
        ...
        items.forEach(function (item) {
            const sugItem = item.sugerencia && item.sugerencia.tiene_dosis ? item.sugerencia.sugerido : '';
            const sug = (maxInsumos === 1 && sugerenciaActual && sugerenciaActual.tiene_dosis)
                ? sugerenciaActual.sugerido
                : sugItem;
            const prev = estado.insumos.find(function (i) { return Number(i.insumoid) === Number(item.id); });
            const val = prev ? prev.cantidad : sug;
            const hint = item.sugerencia && item.sugerencia.tiene_dosis
                ? '<small class="text-success d-block mt-1">Sugerido para ' + fmtNum(item.sugerencia.superficie_ha) + ' ha: '
                    + fmtNum(item.sugerencia.sugerido) + ' ' + (item.sugerencia.unidad || item.unidad) + '</small>'
                : '';
    ...
    function confirmarInsumos() {
        ...
        detalleConfirmadoTipoId = selTipo ? selTipo.value : '';
        limpiarErrorDetalle();
        persistir();
        modalDetalleAbierto = false;
    ...
    function abrirModalParaTipo() {
        ...
        document.getElementById('actDetInsumosAyuda').textContent = (tipoSlugActual === 'material_siembra' ? 1 : 10) === 1
            ? 'Elija un insumo. Calculamos la cantidad según las hectáreas del lote; puede ajustarla.'
            : 'Elija uno o más insumos. Al seleccionar, calculamos cuánto necesita todo el lote según su superficie.';
    ...
    document.getElementById('btnActDetInsumosConfirmar')?.addEventListener('click', function () {
        if (confirmarInsumos()) {
            modalDetalleAbierto = false;
        }
    });
    ...
    formActividad?.addEventListener('submit', function (e) {
        if (!selTipo || !selTipo.value) return;
        const slug = slugDesdeTipoSelect();
        if (!tipoRequiereDetalle(slug)) return;

        if (!detalleEstaCompleto(slug) || detalleConfirmadoTipoId !== selTipo.value) {
            e.preventDefault();
            e.stopPropagation();
            mostrarErrorDetalle(
                slug === 'riego'
                    ? 'Debe elegir y confirmar el tipo de riego en el cuadro.'
                    : 'Debe elegir y confirmar los insumos en el cuadro.'
            );
            abrirModalParaTipo();
            return false;
        }
    ...
    try {
        if (hiddenJson && hiddenJson.value) {
            ...
            if (selTipo && selTipo.value && detalleEstaCompleto(slugDesdeTipoSelect())) {
                detalleConfirmadoTipoId = selTipo.value;
            }
            persistir();
        }
    } catch (err) { /* ignore */ }
