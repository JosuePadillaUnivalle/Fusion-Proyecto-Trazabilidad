...
<style>
#mapaPedidoEntrega { height: 380px; min-height: 380px; border-radius: 10px; border: 1px solid #d1e7d4; }
#mapaTrasladoMayorista { height: 380px; min-height: 380px; border-radius: 10px; border: 1px solid #e9e5ff; }
#mapaTrasladoMayorista-wrap { position: relative; }
#mapaPdvDistribucion { height: 380px; min-height: 380px; border-radius: 10px; border: 1px solid #fed7aa; }
...
@section('content')
    <div class="card card-outline card-primary border-0 shadow-sm modulo-env" style="border-radius:14px;">
        ...
        <div class="card-body">
            ...
            <div id="env-wizard-unificado" class="env-wizard-wrap" data-tema="planta">
            ...
            <form action="{{ route('pedidos.store') }}" method="POST" id="form-pedido">
                ...
                <div class="form-row mb-3">
                    ...
                    <div class="form-group col-md-3 mb-md-0">
                        ...
                        <input type="date" name="fechaEntregaDeseada" class="form-control form-control-sm" value="{{ old('fechaEntregaDeseada') }}" required>
            ...
            <div id="flujo-mayorista" class="d-none">
            ...
            <div id="flujo-punto-venta" class="d-none">
                @can('pedidos_distribucion.create')
                    @include('pedidos.partials.flujo-distribucion-pdv')
                @else
                    <div class="alert alert-warning mb-0">
                        Su rol no tiene permiso para crear pedidos de distribución. Contacte al administrador.
                    </div>
                @endcan
            </div>
            ...
            <div class="env-wizard-nav" id="env-wizard-nav">
                ...
                <div>
                    ...
                    <button type="submit" class="btn btn-success px-4 d-none" id="wizard-btn-guardar-pdv" form="form-pedido-dist-pdv">
                        <i class="fas fa-check mr-1"></i> Confirmar solicitud
                    </button>
                    @endcan
                </div>
            </div>
...
(function () {
    ...
    function recalcularKgFilaTraslado(fila) {
        ...
        const selLote = fila.querySelector('[data-field="inventario_lote"]');
        const loteRequerido = selLote && !selLote.disabled && selLote.options.length > 1 && !selLote.value;
        ...
        fila.classList.toggle('is-stock-error', !!(loteRequerido || sinStock || excede));
    ...
    function initMapaTraslado() {
        ...
        if (!el || !window.L || mapaTrasladoListo) return;
    ...
    function ensureMapaTraslado() {
        ...
        if (!mapaTrasladoListo && window.L && window.RutaPorCalles) {
            initMapaTraslado();
            return;
        }
        if (stateTraslado.map) {
            setTimeout(function () {
                stateTraslado.map.invalidateSize();
                redrawRutaTraslado();
            }, 200);
    ...
    function ensureMapaPdv() {
        ...
        if (!mapaPdvListo && window.L && window.RutaPorCalles) {
            initMapaPdv();
            return;
        }
        if (statePdv.map) {
