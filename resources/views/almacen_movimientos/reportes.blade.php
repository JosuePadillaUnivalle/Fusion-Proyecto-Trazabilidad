...
.page-rep-almacen .rep-kpi-row { margin-left: -12px; margin-right: -12px; margin-bottom: 1.5rem; }
.page-rep-almacen .rep-kpi-col { padding-left: 12px; padding-right: 12px; margin-bottom: 0; }
.page-rep-almacen .rep-charts-row { margin-left: -12px; margin-right: -12px; margin-top: .5rem; margin-bottom: 1.5rem; }
.page-rep-almacen .rep-charts-col { padding-left: 12px; padding-right: 12px; margin-bottom: 1rem; }
@media (min-width: 992px) {
    .page-rep-almacen .rep-charts-col { margin-bottom: 0; }
}
...
.page-rep-almacen #rep-filtros { scroll-margin-top: 1.25rem; }
...
@php
    ...
    $urlReportes = route($prefijo.'.movimientos.reportes');
    $urlPeriodoRapido = function (string $clave) use ($urlReportes, $almacenId): string {
        $params = array_filter([
            'periodo' => $clave,
            'almacenid' => $almacenId ?: null,
        ]);

        return $urlReportes.'?'.http_build_query($params).'#rep-filtros';
    };
@endphp

<div class="modulo-inv page-rep-almacen">
<script>
if ('scrollRestoration' in history && (window.location.hash === '#rep-filtros' || /[?&]periodo=/.test(window.location.search))) {
    ...
    <div class="card rep-card" id="rep-filtros">
        ...
        <form method="GET" action="{{ $urlReportes }}#rep-filtros" id="formRepFiltros">
            <div class="card-body">
    ...
    <div class="row rep-charts-row">
        <div class="col-lg-6 rep-charts-col">
    ...
    <div class="card rep-card mb-0 mt-1" id="rep-recientes">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div>
...
<script>
(function () {
    var debeAnclarFiltros = window.location.hash === '#rep-filtros'
        || /[?&]periodo=/.test(window.location.search);

    if (!debeAnclarFiltros) {
        return;
    }
    ...
    function anclarFiltros() {
        var el = document.getElementById('rep-filtros');
        if (!el) {
            return;
        }
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', anclarFiltros);
    } else {
        anclarFiltros();
    }
})();
