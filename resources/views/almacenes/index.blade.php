    ...
    <div class="card card-outline card-success card-modulo-main elevation-1">
        ...
        <div id="filtrosAlmacenesPanel" class="filtros-panel collapse">
            ...
            <x-filtros-client-actions>
                ...
                <button type="button" class="btn btn-outline-success btn-sm" id="btnVerMapaAlmacenes">
                ...
                <tbody>
                    @forelse($almacenes as $a)
                        @php
                            ...
                            $tieneRecursos = ($oc['ocupado_kg'] ?? 0) > 0.001;
                        @endphp
                        <tr class="search-item-row"
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-sm btn-actions">
                                    <a href="{{ route(($rutaPrefijo ?? 'almacen-agricola').'.show', $a) }}" class="btn btn-outline-info" title="Ver"><i class="fas fa-eye"></i></a>
                                    @can('inventario.update')
                                    <a href="{{ route(($rutaPrefijo ?? 'almacen-agricola').'.edit', $a) }}" class="btn btn-outline-warning" title="Editar"><i class="fas fa-edit"></i></a>
                                    @endcan
                                    @can('inventario.delete')
                                    @if($tieneRecursos)
                                        <button type="button"
                                                class="btn btn-outline-danger btn-eliminar-almacen-bloqueado"
                                                title="Vaciar el almacén antes de eliminar"
                ...
                <div class="col-md-6 col-xl-4 mb-3 search-item"
                    ...
                    <div class="card almacen-card h-100 mb-0">
                        ...
                        <div class="card-footer bg-light d-flex justify-content-end py-2">
                            ...
                            @if($tieneRecursos)
                                <button type="button"
                                        class="btn btn-sm btn-outline-danger btn-eliminar-almacen-bloqueado"
                                        title="Vaciar el almacén antes de eliminar"
                                        data-mensaje="{{ $mensajeEliminarConRecursos }}">
                                    <i class="fas fa-trash"></i>
                                </button>
...
$(function () {
    ...
    $('#btnAplicarFiltros').on('click', function () {
        ...
        if (busqueda && window.AlmacenesIndexMapa) {
            if ($('#modalMapaAlmacenesIndex').hasClass('show')) {
                window.AlmacenesIndexMapa.buscar(busqueda);
            } else {
                window.AlmacenesIndexMapa.abrir(busqueda);
            }
        }
    });
    ...
    $(document).on('click', '.btn-eliminar-almacen-bloqueado', function () {
        Swal.fire({
            icon: 'error',
            title: 'No se puede eliminar',
            text: $(this).data('mensaje') || 'Debe vaciar el almacén primero para poder eliminarlo.',
