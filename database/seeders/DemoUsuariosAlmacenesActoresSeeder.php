...
use App\Support\AlmacenDemoReferencia;
...
class DemoUsuariosAlmacenesActoresSeeder extends Seeder
{
    ...
    private function seedAlmacenes(): ?Almacen
    {
        ...
        $items = [
            AlmacenDemoReferencia::definicionCentral(),
            AlmacenDemoReferencia::definicionNorte(),
            AlmacenDemoReferencia::definicionPlanta(),
        ];

        $central = null;
        foreach ($items as $item) {
            $tipoKey = $item['tipo'];
            $nombre = AlmacenNombreCatalogo::nombreSemilla(
                $item['ambito'],
                $item['semilla'],
                $item['lat'],
                $item['lng'],
            ...
            $almacen = Almacen::updateOrCreate(
                ['descripcion' => $item['semilla']],
                [
                    'nombre' => $nombre,
                    'ubicacion' => AlmacenDemoReferencia::ubicacionConGps($item['lat'], $item['lng'], $item['zona']),
                    'capacidad' => 50000,
            ...
            if ($item['semilla'] === AlmacenDemoReferencia::SEMILLA_CENTRAL) {
                $central = $almacen;
            }
        }
