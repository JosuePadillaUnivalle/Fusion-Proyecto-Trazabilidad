...
class RutaDistribucionTest extends TestCase
{
    ...
    private function usuarioJefeMayorista(): Usuario
    {
        $this->seed(RolePermissionSeeder::class);
        Role::findOrCreate('jefe_mayorista', 'web');

        $user = Usuario::create([
            'nombre' => 'Jefe',
            'apellido' => 'Mayorista',
            'email' => 'jefe.mayorista.ruta@test.local',
            'nombreusuario' => 'jefe_mayorista_ruta',
            'passwordhash' => Hash::make('secret'),
            'role' => 'jefe_mayorista',
            'fecharegistro' => now(),
            'fechamodificacion' => now(),
        ...
        $user->assignRole('jefe_mayorista');

        return $user;
    }

    public function test_mayorista_no_puede_ver_planificacion_distribucion(): void
    {
        $user = $this->usuarioJefeMayorista();
        $this->actingAs($user);
    ...
    public function test_crea_ruta_con_pedidos_confirmados(): void
    {
        ...
        $chofer = Usuario::create([
            'nombre' => 'Chofer',
            'apellido' => 'Mayorista',
            'email' => 'chofer.mayorista@test.local',
            'nombreusuario' => 'chofer_mayorista',
            'passwordhash' => Hash::make('secret'),
            'role' => 'transportista',
            'fecharegistro' => now(),
            'activo' => true,
