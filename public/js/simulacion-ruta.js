...
(function () {
    ...
    function actualizarPanelVivo(container, data) {
        ...
        if (pct) {
            ...
            pct.className = `h4 mb-0 sim-live-pct sim-live--${completada ? 'completado' : 'activo'}`;
        }
        if (bar) {
            bar.style.width = `${progreso}%`;
            bar.setAttribute('aria-valuenow', String(progreso));
            bar.className = `progress-bar sim-live-bar sim-live--${completada ? 'completado' : 'activo'}`;
        }
        ...
        if (badge) {
            ...
            badge.className = `badge sim-live-badge sim-live--${completada ? 'completado' : 'activo'}`;
        }
    }
