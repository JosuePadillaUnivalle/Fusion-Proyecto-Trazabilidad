...
final class CultivoSiembraCatalogo
{
    ...
    public static function rendimientoCosechaPorNombreCultivo(?string $nombre): ?array
    {
        ...
        $tabla = [
            ...
            'pepino' => ['kg_ha' => 35000.0, 'etiqueta' => '35.000 kg/ha'],
            'brócoli' => ['kg_ha' => 12000.0, 'etiqueta' => '12.000 kg/ha'],
            'brocoli' => ['kg_ha' => 12000.0, 'etiqueta' => '12.000 kg/ha'],
            'espinaca' => ['kg_ha' => 10000.0, 'etiqueta' => '10.000 kg/ha'],
    ...
    public static function rendimientoCosechaKgHaDesdeInsumo(Insumo $insumo): ?float
    {
        ...
        $fallback = self::rendimientoCosechaPorNombreCultivo(
            \App\Support\PedidoCatalogo::cultivoDesdeInsumo($insumo)
        );

        return $fallback['kg_ha'] ?? null;
    }

    public static function sugerenciaParaInsumo(Insumo $insumo, float $superficieHa): array
    {
        $porHa = $insumo->dosis_por_ha;
        $unidad = $insumo->dosis_unidad;

        if ($porHa === null || (float) $porHa <= 0 || $unidad === null || trim((string) $unidad) === '') {
            $referencia = InsumoDosisReferenciaCatalogo::paraInsumo($insumo);
            if ($referencia !== null) {
                $porHa = $referencia['por_ha'];
                $unidad = $referencia['unidad'];
                $etiqueta = $referencia['etiqueta_unidad'];
            } else {
