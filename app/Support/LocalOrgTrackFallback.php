...
final class LocalOrgTrackFallback
{
    ...
    public const AMBITO_EMPAQUE_AGRICOLA = 'agricola';

    public const AMBITO_EMPAQUE_PLANTA = 'planta';

    /** Empaques exclusivos de producto terminado / planta (no hortícola). */
    private const NOMBRES_EMPAQUE_SOLO_PLANTA = ['Lata', 'Frasco', 'Bidón', 'Pouch'];

    /**
     * Catálogo de tipos de empaque para mandar-envio (medidas opcionales rellenadas en vacío si no existen en BD).
     *
     * @param  self::AMBITO_EMPAQUE_*|null  $ambito
     */
    public static function tiposEmpaqueCatalogList(?string $ambito = null): array
    {
        if (! Schema::hasTable('tipo_empaque')) {
            return [];
        }

        $items = TipoEmpaque::query()
            ->where('activo', true)
            ->orderBy('nombre')
            ->get()
            ->map(function (TipoEmpaque $e) {
                return [
                    ...
                    'ayuda' => self::ayudaTipoEmpaque((string) $e->nombre, (string) ($e->descripcion ?? '')),
                    'largo' => $e->largo_cm ?? '',
                    'ancho' => $e->ancho_cm ?? '',
                    'alto' => $e->alto_cm ?? '',
                    'tara' => $e->tara_kg ?? '',
                    'capacidad' => $e->capacidad_unidades ?? '',
                    'unidades_por_pallet' => $e->unidades_por_pallet ?? '',
                ];
            })
            ->values()
            ->all();

        if ($ambito === self::AMBITO_EMPAQUE_AGRICOLA) {
            return array_values(array_filter(
                $items,
                fn (array $row) => ! in_array($row['nombre'], self::NOMBRES_EMPAQUE_SOLO_PLANTA, true)
            ));
        }

        if ($ambito === self::AMBITO_EMPAQUE_PLANTA) {
            $compartidos = ['Bolsa plástica', 'Caja de cartón'];

            return array_values(array_filter(
                $items,
                fn (array $row) => in_array($row['nombre'], self::NOMBRES_EMPAQUE_SOLO_PLANTA, true)
                    || in_array($row['nombre'], $compartidos, true)
            ));
        }
    ...
    /** Texto breve para el usuario en el formulario de envío. */
    public static function ayudaTipoEmpaque(string $nombre, string $descripcion = ''): string
    {
        $mapa = [
            'Caja de cartón' => 'Caja rígida estándar para verduras a granel o en manojos; la más usada en cosecha.',
            'Caja plástica' => 'Caja reutilizable, más resistente y pesada; ideal para viajes repetidos.',
            'Bolsa plástica' => 'Bolsa flexible para producto suelto o de bajo volumen (ej. hojas, tubérculos chicos).',
            'Bandeja' => 'Bandeja liviana (EPS) para fruta o verdura delicada; pocas piezas por envase.',
            'Canasta' => 'Canasta plástica apilable; buena para manojos o unidades medianas.',
            'Saco' => 'Saco grande para tubérculos, cebollas o producto a granel pesado.',
            'Pallet' => 'Base de madera/plástico; agrupa muchas cajas o bultos para carga masiva.',
        ];

        return $mapa[$nombre] ?? ($descripcion !== '' ? $descripcion : 'Contenedor para transportar y contar el producto.');
    }

    /**
     * Verduras / cultivos AgroFusion para el formulario de envío (sin categoría OrgTrack).
     *
     * @return list<array{id: int, id_insumo: int, nombre: string, peso_promedio: string}>
     */
    public static function productosVerdurasCatalogList(): array
    {
        if (! Schema::hasTable('insumo')) {
            return [];
        }

        $ids = array_keys(InsumoCatalogo::insumosVerdurasParaLogistica());
        if ($ids === []) {
            return [];
        }

        return Insumo::query()
            ->whereIn('insumoid', $ids)
            ->orderBy('nombre')
            ->get()
            ->map(fn (Insumo $i) => [
                'id' => (int) $i->insumoid,
                'id_insumo' => (int) $i->insumoid,
    ...
    public static function tamanoConteoCatalogList(?int $insumoId = null): array
    {
        if (! Schema::hasTable('catalogo_tamano_conteo')) {
            return [];
        }

        $q = \App\Models\CatalogoTamanoConteo::query()
            ->with('insumo:insumoid,nombre')
            ->where('activo', true)
            ->orderBy('nombre');

        if ($insumoId) {
            $q->where('insumoid', $insumoId);
        }
