...
class EstadoLoteCatalogo
{
    ...
    public static function colorMapa(string $slug): string
    {
        return match ($slug) {
            'planificado' => '#6c757d',
            'sembrado' => '#007bff',
            'en_crecimiento' => '#28a745',
            'listo_para_cosecha' => '#17a2b8',
            'cosechado' => '#ffc107',
            'finalizado' => '#343a40',
            default => '#6c757d',
        };
    ...
    public static function leyendaMapa(): array
    {
        return collect(self::ESTADOS)
            ...
            ->map(fn (array $meta, string $slug) => [
                'slug' => $slug,
                'label' => $meta['label'],
                'color' => self::colorMapa($slug),
            ])
            ->values()
            ->all();
    }

    /** @return array<string, string> clave normalizada (sin acentos) → color hex */
    public static function coloresMapaJs(): array
    {
        $colores = [];

        foreach (self::ESTADOS as $slug => $meta) {
            $color = self::colorMapa($slug);
            $colores[mb_strtolower($meta['label'])] = $color;
        }

        foreach (self::LEGACY_MAP as $legacy => $slug) {
            if ($legacy === 'en descanso') {
                continue;
            }
            $colores[$legacy] = self::colorMapa($slug);
        }

        $colores['disponible'] = self::colorMapa('planificado');
        $colores['en preparacion'] = self::colorMapa('planificado');
        $colores['en produccion'] = self::colorMapa('en_crecimiento');
        $colores['certificado'] = self::colorMapa('listo_para_cosecha');
        $colores['no conforme'] = self::colorMapa('listo_para_cosecha');
