...
class LoteTrazabilidadService
{
    ...
    private function milestoneSiembra(Lote $lote): bool
    {
        ...
        return $this->actividadCompletadaConKeywords($lote, ['siembra']);
    }
    ...
    public function buildEventos(Lote $lote): Collection
    {
        $lote->loadMissing([
            ...
            'loteInsumos.insumo.unidadMedida',
            'loteInsumos.usuario',
            'actividades.tipoActividad',
            'actividades.usuario',
        ...
        foreach ($lote->historialEstados as $historial) {
            ...
            $slugEstado = EstadoLoteCatalogo::slugFromNombre($estadoNombre);
            if ($slugEstado === 'sembrado') {
                continue;
            }
            if ($this->historialEnCrecimientoEsRedundante($lote, $historial)) {
                continue;
            }
        ...
        foreach ($lote->loteInsumos as $insumo) {
            // Insumos ligados a una actividad ya se muestran en la tarjeta de esa actividad.
            if ($insumo->actividadid) {
                continue;
            }
        ...
        foreach ($lote->actividades as $actividad) {
            ...
            $descripcionAct = $this->descripcionEventoActividad($actividad, $lote->loteInsumos);
            $evidenciaAct = $actividad->fechafin !== null
                ? EvidenciaFoto::urlDesdePath($actividad->evidencia_foto_path ?? null)
                : null;
            $fechaActividad = $actividad->fechafin ?? $actividad->fechainicio;
            $eventos->push($this->evento(
                $fechaActividad,
                'actividad',
                $faseAct,
        ...
        foreach ($lote->actividades as $actividad) {
            ...
            $fechaSiembra = $actividad->fechafin ?? $actividad->fechainicio;
            $nombreSiembra = trim(($actividad->usuario->nombre ?? '').' '.($actividad->usuario->apellido ?? ''))
                ?: ($actividad->usuario->nombre ?? null);

            if (! $this->loteTieneHistorialEnCrecimientoPorSiembra($lote)) {
                $eventos->push($this->evento(
                    $fechaSiembra,
                    'estado',
                    'en_crecimiento',
                    'Estado cambiado a: En crecimiento',
                    'Actividad «Siembra» completada',
                    $nombreSiembra,
                    'exchange-alt',
                    'info'
                ));
            ...
            $eventos->push($this->evento(
                ...
                true,
                null,
                self::FASES['en_crecimiento']['label'],
                $evidenciaSiembra,
            ));
        }
    ...
    {
        $lote->load(['usuario', 'cultivo', 'estadoTipo', 'insumoSemilla', 'catalogoTamanoConteo.tipoEmpaque']);
        ...
        $planificacionEstimada = app(\App\Services\PlanificacionCosechaService::class)->estimacionDesdeLote($lote);

        return compact('lote', 'estadisticas', 'estadoClass', 'planificacionEstimada');
    }
    ...
    private function actividadSiembraCompletada(Lote $lote): ?Actividad
    {
        $lote->loadMissing('actividades.tipoActividad');

        return $lote->actividades->first(function (Actividad $actividad) {
            $nombre = mb_strtolower(trim($actividad->tipoActividad->nombre ?? ''));
    ...
    private function loteTieneHistorialEnCrecimientoPorSiembra(Lote $lote): bool
    {
        return $lote->historialEstados->contains(function ($historial) {
            if (EstadoLoteCatalogo::slugFromNombre($historial->estadoTipo->nombre ?? '') !== 'en_crecimiento') {
                return false;
            }
    ...
    private function historialEnCrecimientoEsRedundante(Lote $lote, $historial): bool
    {
        if (EstadoLoteCatalogo::slugFromNombre($historial->estadoTipo->nombre ?? '') !== 'en_crecimiento') {
            return false;
        }
        ...
        $obs = mb_strtolower((string) ($historial->observaciones ?? ''));
        $esDesdeMantenimiento = str_contains($obs, 'riego')
            || str_contains($obs, 'fertiliz')
            || str_contains($obs, 'plaga')
            || str_contains($obs, 'fumig');

        if (! $esDesdeMantenimiento) {
            return false;
        }

        $siembra = $this->actividadSiembraCompletada($lote);
        if ($siembra?->fechafin && $historial->fecha_cambio) {
            return $this->parseFechaApp($siembra->fechafin)->lte($this->parseFechaApp($historial->fecha_cambio));
        }

        return $lote->fechasiembra !== null;
    }

    /**
     * Texto unificado para fertilización, control de plagas, riego, etc. (sin tarjeta insumo aparte).
     *
     * @param  Collection<int, \App\Models\LoteInsumo>  $loteInsumos
     */
    private function descripcionEventoActividad(Actividad $actividad, Collection $loteInsumos): string
    {
        $aplicaciones = $loteInsumos
            ->where('actividadid', (int) $actividad->actividadid)
            ->values();

        if ($aplicaciones->isNotEmpty()) {
            $partes = $aplicaciones->map(function ($fila) {
                $nombre = trim((string) ($fila->insumo->nombre ?? 'Insumo'));
                $cantidad = number_format((float) $fila->cantidadusada, 2, '.', '');
                $unidad = trim((string) (
                    $fila->insumo->unidadMedida->abreviatura
                    ?? $fila->insumo->unidadMedida->nombre
                    ?? ''
                ));
        ...
        if (is_array($detalle)) {
            ...
            if ($modo === 'insumos') {
                ...
                foreach ($detalle['insumos'] ?? [] as $fila) {
                    ...
                    $partes[] = $unidad !== ''
                        ? $nombre.' · '.number_format($cant, 2, '.', '').' '.$unidad
                        : $nombre.' · '.number_format($cant, 2, '.', '');
                }
                if ($partes !== []) {
                    return 'Aplicó: '.implode('; ', $partes);
                }
        ...
        $tipoNombre = trim((string) ($actividad->tipoActividad->nombre ?? ''));

        if ($descripcion !== '' && $tipoNombre !== ''
            && str_starts_with(mb_strtolower($descripcion), mb_strtolower($tipoNombre).' — ')) {
            return trim(mb_substr($descripcion, mb_strlen($tipoNombre) + 3));
        }
    ...
    public function datosFormularioEnvioAlmacen(Lote $lote): array
    {
        ...
        $presentacionCosecha = $produccion
            ? app(\App\Services\CosechaPresentacionService::class)->paraProduccion($produccion, $lote)
            : null;
        ...
        return [
            ...
            'presentacion_cosecha_almacen' => $presentacionCosecha,
