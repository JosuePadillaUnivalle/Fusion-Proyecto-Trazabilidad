...
class ActividadInsumoService
{
    ...
    public function validarDetalle(array $detalle, ?string $tipoActividadNombre): array
    {
        if (ActividadDetalleCatalogo::esRiego($tipoActividadNombre)) {
            $riego = is_array($detalle['riego'] ?? null) ? $detalle['riego'] : [];
            $tipoRiego = trim((string) ($riego['key'] ?? ''));
            if ($tipoRiego === '') {
    ...
    public function listarInsumosParaModal(string $tipoSlug, ?Lote $lote = null): array
    {
        ...
        if ($tipoSlug === 'material_siembra' && $insumoPlanificadoId) {
            ...
            return $coleccion->map(fn (Insumo $i) => self::mapearInsumoModal($i, $lote))->values()->all();
        }
        ...
        if ($tipoSlug === 'material_siembra' && $referenciaNombre) {
            ...
            if ($filtrados->isNotEmpty()) {
                return $filtrados->map(fn (Insumo $i) => self::mapearInsumoModal($i, $lote))->values()->all();
            }
        }

        return $query->get()->map(fn (Insumo $i) => self::mapearInsumoModal($i, $lote))->values()->all();
    }

    /** @return array<string, mixed> */
    private static function mapearInsumoModal(Insumo $i, ?Lote $lote = null): array
    {
        $item = [
            'id' => (int) $i->insumoid,
            'nombre' => $i->nombre,
        ...
        if ($lote && (float) $lote->superficie > 0) {
            $item['sugerencia'] = \App\Support\CultivoSiembraCatalogo::sugerenciaParaInsumo($i, (float) $lote->superficie);
        }
