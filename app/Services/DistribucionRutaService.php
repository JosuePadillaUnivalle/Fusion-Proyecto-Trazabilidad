...
class DistribucionRutaService
{
    ...
    public function trayectoPartes(RutaDistribucion $ruta): ?array
    {
        ...
        $carga = $ruta->paradas->first(fn (RutaDistribucionParada $p) => in_array($p->tipo, [
        ...
        return [
            'origen' => $origen ?? 'Centro mayorista',
            'destinos' => $entregas,
        ];
    }
    ...
    public function pedidosListosParaRuta(): Collection
    {
        return PedidoDistribucion::query()
            ->with(['puntoVenta.minorista', 'detalles.insumo.unidadMedida', 'almacenMayoristaOrigen'])
    ...
    /** @deprecated Use asegurarTransportistaMayorista() */
    public function asegurarTransportistaPlanta(int $transportistaId): void
    {
        $this->asegurarTransportistaMayorista($transportistaId);
    }
}
