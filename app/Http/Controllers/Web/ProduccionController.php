...
use App\Support\CosechaEnvioHistorialService;
...
class ProduccionController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $planificacion = app(PlanificacionCosechaService::class);
        $estimacionesPorLote = $lotes->mapWithKeys(function (Lote $lote) use ($planificacion) {
            $est = $planificacion->estimacionUiDesdeLote($lote);

            return $est ? [$lote->loteid => $est] : [];
        })->all();
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'almacenid.required' => 'Debe seleccionar un almacén de destino para la cosecha.',
    ...
    public function show(Produccion $produccion, CosechaEnvioHistorialService $historialEnvios)
    {
        $produccion->load(['lote.usuario', 'lote.insumoSemilla', 'lote.cultivo', 'lote.catalogoTamanoConteo.tipoEmpaque', 'destino', 'unidadMedida', 'almacenamientos.almacen', 'almacenamientos.catalogoTamanoConteo.tipoEmpaque', 'ventas']);

        $historialPlanta = $historialEnvios->historialParaProduccion($produccion);
        $presentacionCosecha = $historialEnvios->presentacionProduccion($produccion);

        return view('producciones.show', compact('produccion', 'historialPlanta', 'presentacionCosecha'));
    }
