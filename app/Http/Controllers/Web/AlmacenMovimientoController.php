...
class AlmacenMovimientoController extends Controller
{
    ...
    public function create(Request $request, string $naturaleza)
    {
        ...
        $insumoQuery = Insumo::query()->with('unidadMedida');
        if ($ctx['ambito'] === AlmacenAmbito::MAYORISTA) {
            $insumoQuery = InsumoCatalogo::aplicarFiltroProductoTerminado($insumoQuery);
        } else {
            $insumoQuery = InsumoCatalogo::aplicarFiltroOperativo($insumoQuery);
        }

        $insumosList = $insumoQuery->orderBy('nombre')->get();
