...
class ExternalApiProxyController extends Controller
{
    ...
    /** Comprueba que AgroFusion responde (no depende de OrgTrack ni de Wi‑Fi del cliente). */
    ...
    {
        if (LocalOrgTrackFallback::productosVerdurasCatalogList() !== []) {
            return response()->json([]);
        }
    ...
    {
        $local = LocalOrgTrackFallback::productosVerdurasCatalogList();
        if ($local !== []) {
    ...
    {
        $local = LocalOrgTrackFallback::tiposEmpaqueCatalogList();
        if ($local !== []) {
            return response()->json($local);
        }
    ...
    public function getTamanoConteo(Request $request)
    {
        $insumoId = $request->filled('insumoid') ? (int) $request->input('insumoid') : null;
        $local = LocalOrgTrackFallback::tamanoConteoCatalogList($insumoId);
        if ($local !== []) {
            return response()->json($local);
        }
    ...
    {
        if (\Illuminate\Support\Facades\Schema::hasTable('tipo_transporte')) {
            $rows = \App\Models\TipoTransporte::query()->orderBy('nombre')->get()->map(fn ($t) => [
                'id' => (int) $t->tipotransporteid,
