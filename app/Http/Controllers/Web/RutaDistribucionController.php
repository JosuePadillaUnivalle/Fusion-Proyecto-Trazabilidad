...
class RutaDistribucionController extends Controller
{
    ...
    public function empezarRuta(RutaDistribucion $ruta, SimulacionRutaService $simulacion): RedirectResponse
    {
        ...
        $pedido = $ruta->pedidos()->orderByDesc('pedidodistribucionid')->first();
        if ($pedido !== null && UsuarioRol::puedeMarcarEnRutaDistribucion($user, $ruta)) {
            $redirect = redirect()
                ->route('punto-venta.pedidos.show', ['pedido' => $pedido, 'paso' => 4])
                ->with('success', 'Ruta marcada en marcha. El pedido ya está en camino al punto de venta.');
