...
class EstadoLoteInsumoController extends Controller
{
    ...
    {
        $estados = EstadoLoteInsumo::orderBy('estadoloteinsumoid', 'desc')
            ->paginate(15);
    ...
    public function store(Request $request)
    {
        ...
        return redirect()
            ->route('estado-lote-insumos.index')
            ->with('success', 'Estado de insumo creado.');
    }

    public function show(EstadoLoteInsumo $estadoLoteInsumo)
    {
        return view('estado_lote_insumos.show', compact('estadoLoteInsumo'));
    }

    public function edit(EstadoLoteInsumo $estadoLoteInsumo)
    {
        return view('estado_lote_insumos.edit', compact('estadoLoteInsumo'));
    }
    ...
    public function update(Request $request, EstadoLoteInsumo $estadoLoteInsumo)
    {
        ...
        return redirect()
            ->route('estado-lote-insumos.index')
            ->with('success', 'Estado de insumo actualizado.');
    }
    ...
    public function destroy(EstadoLoteInsumo $estadoLoteInsumo)
    {
        ...
        return redirect()
            ->route('estado-lote-insumos.index')
            ->with('success', 'Estado de insumo eliminado.');
    }
}